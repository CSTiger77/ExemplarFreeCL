...
from lib.config import CKDF_DFCL_cfg, update_config
...
def parse_args():
    parser = argparse.ArgumentParser(description="codes for CKDF-DFCL")

    parser.add_argument(
        "--cfg",
        help="decide which cfg to use",
        required=False,
        default="./configs/CKDF_DFCL_cifar10.yaml",
        # default="./configs/CKDF_DFCL_cifar100.yaml",
        # default="./configs/CKDF_DFCL_tiny.yaml",
        type=str,
    )
    parser.add_argument(
        "opts",
...
if __name__ == "__main__":
    args = parse_args()
    update_config(CKDF_DFCL_cfg, args)
    logger, log_file = create_logger(CKDF_DFCL_cfg, "log")
    ...
    order_class_list = [52, 1, 30, 96, 19, 14, 43, 37, 66, 3, 79, 41, 38, 68, 2, 60, 53, 95, 74, 92, 26, 59, 46, 90, 70,
                        ...
                        28, 64, 15, 9]
    # split_seleted_data = split_classes_per_task(order_class_list, tasks=CKDF_DFCL_cfg.DATASET.all_tasks)
    split_seleted_data = None
    dataset_split_handler = eval(CKDF_DFCL_cfg.DATASET.dataset)(CKDF_DFCL_cfg, split_selected_data=split_seleted_data)
    if CKDF_DFCL_cfg.availabel_cudas:
        os.environ['CUDA_VISIBLE_DEVICES'] = CKDF_DFCL_cfg.availabel_cudas
        device_ids = [i for i in range(len(CKDF_DFCL_cfg.availabel_cudas.strip().split(',')))]
        print(device_ids)

    midfcl_handler = MI_DFCL_handler(dataset_split_handler, CKDF_DFCL_cfg, logger)
