...
from lib.approach.FeTrIL import FeTrIL_handler
from lib.dataset import *
from lib.config import FeTrIL_cfg, update_config
...
def parse_args():
    ...
    parser.add_argument(
        ...
        required=False,
        # default="./configs/FeTrIL_cifar10.yaml",
        default="./configs/FeTrIL_cifar100.yaml",
        # default="./configs/FeTrIL_tiny.yaml",
        # default="./configs/FeTrIL_imagenet.yaml",
        type=str,
    )
    parser.add_argument(
        "opts",
...
if __name__ == "__main__":
    args = parse_args()
    update_config(FeTrIL_cfg, args)
    logger, log_file = create_logger(FeTrIL_cfg, "log")
    ...
    if FeTrIL_cfg.seed == 10:
        if "CIFAR100" == FeTrIL_cfg.DATASET.dataset_name:
            order_class_list = [19, 14, 43, 37, 66, 3, 79, 41, 38, 68, 2, 1, 60, 53, 95, 74, 92, 26, 59, 46, 90, 70, 50,
                                44,76,55, 21, 61, 6, 63, 42, 34, 84, 52, 35, 39, 45, 4, 5, 48, 32, 20, 83, 58, 47, 80, 17, 67,
                                ...
                                28,64,15, 9]
        elif "CIFAR10" == FeTrIL_cfg.DATASET.dataset_name:
            order_class_list = [8, 2, 5, 6, 3, 1, 0, 7, 4, 9]
        elif "tiny" in FeTrIL_cfg.DATASET.dataset_name:
            ...
            raise ValueError(f"FeTrIL_cfg.DATASET.dataset_name")
    elif FeTrIL_cfg.seed == 100:
        if "CIFAR100" == FeTrIL_cfg.DATASET.dataset_name:
            order_class_list = [37, 62, 26, 41, 35, 25, 36, 33, 77, 21, 85, 50, 92, 69, 96, 78, 72, 5, 40, 11, 29, 83,
                                82,43, 28, 22, 23, 90, 86, 20, 32, 6, 3, 12, 51, 84, 73, 64, 54, 68, 75, 74, 57, 42, 76,
                                99,17, 93, 63, 0, 18, 44, 38, 45, 39, 70, 94, 30, 71, 46, 56, 80, 91, 88, 19, 81, 55, 89,
                                61,65, 47, 49, 7, 97, 59, 95, 13, 1, 31, 4, 27, 2, 9, 16, 58, 60, 15, 98, 34, 14, 66, 53,
                                52,10, 48, 79, 87, 67, 24, 8]
        elif "CIFAR10" == FeTrIL_cfg.DATASET.dataset_name:
            ...
            raise ValueError(f"FeTrIL_cfg.DATASET.dataset_name")
    elif FeTrIL_cfg.seed == 0:
        order_class_list = None
    else:
        order_class_list = None
        # raise ValueError(f"EARS_DFCL_cfg.seed == 0, 10, 100")
    # split_seleted_data = {0: [52, 1, 30, 96], 1: [4, 5, 6, 7], 2: [8, 9, 10, 11], 3: [12, 13, 14, 15], 4: [16, 17, 18, 19], 5: [20, 21, 22, 23], 6: [24, 25, 26, 27], 7: [28, 29, 2, 31], 8: [32, 33, 34, 35], 9: [36, 37, 38, 39], 10: [40, 41, 42, 43], 11: [44, 45, 46, 47], 12: [48, 49, 50, 51], 13: [0, 53, 54, 55], 14: [56, 57, 58, 59], 15: [60, 61, 62, 63], 16: [64, 65, 66, 67], 17: [68, 69, 70, 71], 18: [72, 73, 74, 75], 19: [76, 77, 78, 79], 20: [80, 81, 82, 83], 21: [84, 85, 86, 87], 22: [88, 89, 90, 91], 23: [92, 93, 94, 95], 24: [3, 97, 98, 99]}
    split_seleted_data = split_classes_per_task(order_class_list, tasks=FeTrIL_cfg.DATASET.all_tasks)
    dataset_split_handler = eval(FeTrIL_cfg.DATASET.dataset)(FeTrIL_cfg, split_selected_data=split_seleted_data)
    if FeTrIL_cfg.availabel_cudas:
        os.environ['CUDA_VISIBLE_DEVICES'] = FeTrIL_cfg.availabel_cudas
        device_ids = [i for i in range(len(FeTrIL_cfg.availabel_cudas.strip().split(',')))]
        print(device_ids)

    feTrIL_handler = FeTrIL_handler(dataset_split_handler, FeTrIL_cfg, logger)
    if "imagenet" in FeTrIL_cfg.DATASET.dataset_name and \
            "tiny" not in FeTrIL_cfg.DATASET.dataset_name:
        feTrIL_handler.feTrIL_train_main_for_local_dataset()
    else:
        feTrIL_handler.feTrIL_train_main()
