...
from lib.config import RDFCL_cfg, update_config
...
def parse_args():
    parser = argparse.ArgumentParser(description="codes for R-DFCL")

    parser.add_argument(
        "--cfg",
        help="decide which cfg to use",
        required=False,
        # default="./configs/RDFCL_cifar10.yaml",
        default="./configs/RDFCL_cifar100.yaml",
        # default="./configs/RDFCL_tiny.yaml",
        # default="./configs/RDFCL_imagenet.yaml",
        type=str,
    )
    parser.add_argument(
        "opts",
...
if __name__ == "__main__":
    args = parse_args()
    update_config(RDFCL_cfg, args)
    logger, log_file = create_logger(RDFCL_cfg, "log")
    ...
    if RDFCL_cfg.seed == 10:
        if "CIFAR100" == RDFCL_cfg.DATASET.dataset_name:
            order_class_list = [19, 14, 43, 37, 66, 3, 79, 41, 38, 68, 2, 1, 60, 53, 95, 74, 92, 26, 59, 46, 90, 70, 50,
                                ...
                                64,
                                15, 9]
        elif "CIFAR10" == RDFCL_cfg.DATASET.dataset_name:
            order_class_list = [8, 2, 5, 6, 3, 1, 0, 7, 4, 9]
        elif "tiny" in RDFCL_cfg.DATASET.dataset_name:
            ...
            raise ValueError(f"RDFCL_cfg.DATASET.dataset_name")
    elif RDFCL_cfg.seed == 100:
        if "CIFAR100" == RDFCL_cfg.DATASET.dataset_name:
            ...
            raise ValueError(f"RDFCL_cfg.DATASET.dataset_name")
    elif RDFCL_cfg.seed == 0:
        order_class_list = None
    else:
        raise ValueError(f"EARS_DFCL_cfg.seed == 0, 10, 100")
    # split_seleted_data = {0: [52, 1, 30, 96], 1: [4, 5, 6, 7], 2: [8, 9, 10, 11], 3: [12, 13, 14, 15], 4: [16, 17, 18, 19], 5: [20, 21, 22, 23], 6: [24, 25, 26, 27], 7: [28, 29, 2, 31], 8: [32, 33, 34, 35], 9: [36, 37, 38, 39], 10: [40, 41, 42, 43], 11: [44, 45, 46, 47], 12: [48, 49, 50, 51], 13: [0, 53, 54, 55], 14: [56, 57, 58, 59], 15: [60, 61, 62, 63], 16: [64, 65, 66, 67], 17: [68, 69, 70, 71], 18: [72, 73, 74, 75], 19: [76, 77, 78, 79], 20: [80, 81, 82, 83], 21: [84, 85, 86, 87], 22: [88, 89, 90, 91], 23: [92, 93, 94, 95], 24: [3, 97, 98, 99]}
    split_seleted_data = split_classes_per_task(order_class_list, tasks=RDFCL_cfg.DATASET.all_tasks)
    dataset_split_handler = eval(RDFCL_cfg.DATASET.dataset)(RDFCL_cfg, split_selected_data=split_seleted_data)
    if RDFCL_cfg.availabel_cudas:
        os.environ['CUDA_VISIBLE_DEVICES'] = RDFCL_cfg.availabel_cudas
        device_ids = [i for i in range(len(RDFCL_cfg.availabel_cudas.strip().split(',')))]
        print(device_ids)

    midfcl_handler = MI_DFCL_handler(dataset_split_handler, RDFCL_cfg, logger)
